...
handelsubmitform = () => {
  ...
  componentDidMount() {
    ...
    axios({
      method: 'get',
      url: this.state.url + '?State=Work%20from%20home&SortByDate=1&page=1&size=25',
      headers: Header,
    })
      .then(data => {
        this.setState({
...
Foto
          ...
          Note
          <InputGroup className="mb-3">
          <Input type="text" onChange={this.handleNote} value={this.state.Note}  placeholder="Catatan"/>
          </InputGroup>
              ...
              <Col xs="12" lg="12">
                <Card>
                  <CardHeader>
                    <Row>
                      <Col>
                        <i className="fa fa-user" /> <b>&nbsp;Work From Home</b>
                        {/* <Button
                          style={{ marginLeft: 10 }}
                          color="success"
                          className="px-4"
